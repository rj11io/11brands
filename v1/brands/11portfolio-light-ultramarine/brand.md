...
The `11portfolio` site. This file is candidate `ultramarine` in the
11portfolio colour round, which now holds ten light-mode blue variants. None
chosen yet.

## Palette decision

Mode light. Signal `#2B3FBF` on ground `#FAFAFA`: **7.84:1**.

Ultramarine. Blue shifted toward violet, which is the direction with the most
free space left in the family.
...
| --- | --- | --- |
| ground | `#FAFAFA` | 7.84:1 |
| ink (the numeral beside the square) | `#0A0A0A` | 2.42:1 |
| footer grey | `#676767` | 1.45:1 |

Nearest active brand on this ground: `11blog-11cv`, CIEDE2000 **12.5**. Every
...
dE 12.5 from 11cv, a comfortable margin bought by moving off the blue axis
rather than by going darker. The nearest violets are all archived
(`11brands-light-violet` and the `11archive-light-*` purples), so the
direction is clear.
