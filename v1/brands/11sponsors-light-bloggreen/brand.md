...
The `11sponsors` site. This file is candidate `bloggreen` in the first
...
Emerald 400. Requested explicitly: 11blog's exact signal, carried onto a light
ground.
...
| --- | --- | --- |
| ground | `#FAFAFA` | 2.06:1 |
| ink (the numeral beside the square) | `#0A0A0A` | 9.20:1 |
| footer grey | `#676767` | 2.63:1 |

Measured against `11ai`, the brand this half of the round has to clear:
CIEDE2000 **24.7**. Every candidate here is chromatic, so the footer ratio
...
**This candidate fails the contrast floor.** 2.06:1 against `#FAFAFA` misses
the 3:1 minimum for a non-text graphic — `init_brand.py` printed the warning
when it was created.

Unlike the pink counterpart in this round, there is no identity collision:
11blog uses this hex on a **dark** ground (`#0A0A0A`), where it measures
11.6:1 and works well. Moving it to a light ground is what breaks it. The two
brands would share a hex but never share a context.

If it wins, the mark is low-contrast at every size and unusable at 16px
against a white browser tab. `emerald600` in this round is the closest passing
relative if the emerald direction is what appeals. Note the inverted numbers:
this is the only candidate whose ink (9.20:1) and footer (2.63:1) contrast are
high while its ground contrast is low, because it is a bright colour on a
bright ground.
