...
The `11sponsors` site. This file is candidate `green600` in the first
11sponsors colour round: twelve light-mode variants, six pink and six green,
none of them chosen yet.

## Palette decision

Mode light. Signal `#16A34A` on ground `#FAFAFA`: **3.16:1**.

Green 600. The brightest true green that clears the floor at all.
...
| --- | --- | --- |
| ground | `#FAFAFA` | 3.16:1 |
| ink (the numeral beside the square) | `#0A0A0A` | 6.01:1 |
| footer grey | `#676767` | 1.72:1 |

Measured against `11ai`, the brand this half of the round has to clear:
CIEDE2000 **17.0**. Every candidate here is chromatic, so the footer ratio
...
Floor-only at 3.16:1, with only 0.16 of headroom. dE 17.0 from 11ai, the
widest separation in this round, because it is a pure green where 11ai's is a
blue-leaning emerald. Never set it as text.
