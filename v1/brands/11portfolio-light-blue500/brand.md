...
The `11portfolio` site. This file is candidate `blue500` in the 11portfolio
colour round, which now holds ten light-mode blue variants. None chosen yet.

## Palette decision

Mode light. Signal `#3B82F6` on ground `#FAFAFA`: **3.52:1**.

Blue 500. The canonical bright blue, and the closest bright candidate to 11cv.
...
CIEDE2000 **10.9**. Every candidate here is chromatic, so the footer ratio
binds none of them.
...
dE 10.9 from 11cv at 3.52:1. It is the most conventional reading of 'bright
blue', and also the one most likely to be mistaken for 11cv at a glance, since
11cv's blue 600 is literally the next step down the same scale.
