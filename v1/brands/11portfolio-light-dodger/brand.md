...
The `11portfolio` site. This file is candidate `dodger` in the 11portfolio
colour round, which now holds ten light-mode blue variants. None chosen yet.

## Palette decision

Mode light. Signal `#1E90FF` on ground `#FAFAFA`: **3.10:1**.

Dodger blue. The most vivid blue in the round: maximum brightness, minimum
contrast.
...
| --- | --- | --- |
| ground | `#FAFAFA` | 3.10:1 |
| ink (the numeral beside the square) | `#0A0A0A` | 6.12:1 |
| footer grey | `#676767` | 1.75:1 |

Nearest active brand on this ground: `11cv` / `11blog-11cv` (`#2563EB`),
CIEDE2000 **16.2**. Every candidate here is chromatic, so the footer ratio
binds none of them.
...
**3.10:1 barely clears the floor** — 0.10 of headroom, the narrowest pass in
the whole project. Any change to the ground breaks it, and it must never be
set as text. In exchange it is the brightest candidate that passes at all, and
at dE 16.2 it is well clear of 11cv.
