...
The `11sponsors` site. This file is candidate `fuchsia600` in the first
11sponsors colour round: twelve light-mode variants, six pink and six green,
none of them chosen yet.

## Palette decision

Mode light. Signal `#C026D3` on ground `#FAFAFA`: **4.51:1**.

Fuchsia 600. The purple-leaning end of pink, and the brightest candidate that
clears the aim.
...
| --- | --- | --- |
| ground | `#FAFAFA` | 4.51:1 |
| ink (the numeral beside the square) | `#0A0A0A` | 4.20:1 |
| footer grey | `#676767` | 1.20:1 |

Measured against `11support`, the brand this half of the round has to clear:
CIEDE2000 **17.7**. Every candidate here is chromatic, so the footer ratio
...
dE 17.7 from 11support, the second widest in the round. Nearest anywhere is
the archived `11brands-light-fuchsia` (`#A21CAF`) at dE 8.0 — retired, so the
colour is free, but it is the reason this reads as fuchsia rather than as a
new pink.
