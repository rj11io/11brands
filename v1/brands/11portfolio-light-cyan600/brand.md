...
The `11portfolio` site. This file is candidate `cyan600` in the 11portfolio
colour round, which now holds ten light-mode blue variants. None chosen yet.

## Palette decision

Mode light. Signal `#0891B2` on ground `#FAFAFA`: **3.53:1**.

Cyan 600. The brightest cyan-leaning blue that clears the floor, and the
furthest of the bright group from 11cv.
...
| --- | --- | --- |
| ground | `#FAFAFA` | 3.53:1 |
| ink (the numeral beside the square) | `#0A0A0A` | 5.38:1 |
| footer grey | `#676767` | 1.54:1 |

Nearest active brand on this ground: `11cv` / `11blog-11cv` (`#2563EB`),
CIEDE2000 **22.8**. Every candidate here is chromatic, so the footer ratio
binds none of them.
...
dE 22.8 from 11cv, the widest separation in the bright group. Its nearest
relative is `sky700` in this same round at dE 17.1, so it is genuinely its own
option rather than a shade of one. At 3.53:1 it is floor-only: never set it as
text.
