...
The `11gw` site. This file is candidate `rose400` in the 11gw **dark-mode**
round: six variants on `#0A0A0A`, none of them chosen yet. A separate
light-mode round of six exists as `11gw-light-*`.

## Palette decision

Mode dark. Signal `#FB7185` on ground `#0A0A0A`: **7.36:1**.

Rose 400. The pink-leaning option, sitting between 11intel's red and 11brands' pink.
...
| --- | --- | --- |
| ground | `#0A0A0A` | 7.36:1 |
| ink (the numeral beside the square) | `#FAFAFA` | 2.58:1 |
| footer grey | `#A1A1A1` | 1.04:1 |

Nearest brand sharing this ground: `11blog-11brands`, CIEDE2000 **13.0**. Every
...
dE 13.0 from 11brands' pink 400 and further still from 11intel, so it threads between the two. Worth checking against 11brands specifically, since that is now the nearest neighbour rather than 11intel.
