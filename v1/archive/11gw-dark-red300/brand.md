...
The `11gw` site. This file is candidate `red300` in the 11gw **dark-mode**
round: six variants on `#0A0A0A`, none of them chosen yet. A separate
light-mode round of six exists as `11gw-light-*`.

## Palette decision

Mode dark. Signal `#FCA5A5` on ground `#0A0A0A`: **10.43:1**.

Red 300. The palest candidate, and the furthest from 11intel by a wide margin.
...
| --- | --- | --- |
| ground | `#0A0A0A` | 10.43:1 |
| ink (the numeral beside the square) | `#FAFAFA` | 1.82:1 |
| footer grey | `#A1A1A1` | 1.36:1 |

Nearest brand sharing this ground: `11blog-11brands`, CIEDE2000 **18.2**. Every
...
dE 18.2 from its nearest neighbour, the safest separation in the round. The cost is the ink: 1.82:1 is the weakest here, because a pale red on a dark ground is close in lightness to the near-white numeral beside it.
