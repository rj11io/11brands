...
The `11pc` site. This file is candidate `brass` in the first 11pc colour
round: six light-mode yellow variants, none of them chosen yet.

## Palette decision

Mode light. Signal `#B08900` on ground `#FAFAFA`: **3.13:1**.

Brass, yellow 650-ish. The brightest true yellow hue in the round: no orange lean, unlike the two ambers.
...
| --- | --- | --- |
| ground | `#FAFAFA` | 3.13:1 |
| ink (the numeral beside the square) | `#0A0A0A` | 6.06:1 |
| footer grey | `#676767` | 1.73:1 |

Nearest active brand: `11blog-11labs`, CIEDE2000 **27.9**. Every candidate in this
...
Floor-only at 3.13:1, so the same no-type rule applies. Well clear of every active brand (dE 27.9). This is the candidate to pick if the brand should read yellow rather than amber, and the brand accepts a floor-only signal to get there.
