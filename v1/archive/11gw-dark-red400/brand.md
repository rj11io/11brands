...
The `11gw` site. This file is candidate `red400` in the 11gw **dark-mode**
round: six variants on `#0A0A0A`, none of them chosen yet. A separate
light-mode round of six exists as `11gw-light-*`.

## Palette decision

Mode dark. Signal `#F87171` on ground `#0A0A0A`: **7.16:1**.

Red 400. The canonical dark-mode red, and the closest thing here to what 11intel already uses.
...
| --- | --- | --- |
| ground | `#0A0A0A` | 7.16:1 |
| ink (the numeral beside the square) | `#FAFAFA` | 2.65:1 |
| footer grey | `#A1A1A1` | 1.07:1 |

Nearest brand sharing this ground: `11blog-11intel`, CIEDE2000 **9.8**. Every
...
dE 9.8 from 11intel is the tightest pairing in the round and the only one under 10. Both are dark-ground reds, so they share a context directly. Choosing this means accepting that 11gw and 11intel read as siblings; if that is not wanted, pick almost anything else here.
