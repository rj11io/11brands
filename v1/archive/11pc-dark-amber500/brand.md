...
The `11pc` site. This file is candidate `amber500` in the 11pc **dark-mode**
round: six variants on `#0A0A0A`, none of them chosen yet. A separate
light-mode round of six exists as `11pc-light-*`.

## Palette decision

Mode dark. Signal `#F59E0B` on ground `#0A0A0A`: **9.22:1**.

Amber 500. The warmest and brightest candidate that still clears 11b2b.
...
| --- | --- | --- |
| ground | `#0A0A0A` | 9.22:1 |
| ink (the numeral beside the square) | `#FAFAFA` | 2.06:1 |
| footer grey | `#A1A1A1` | 1.20:1 |

Nearest brand sharing this ground: `11b2b`, CIEDE2000 **10.6**. Every
...
At dE 10.6 from 11b2b this is the tightest pairing in the round, and the two share a ground, so they will genuinely appear in the same context. Comparable to the dE 10.3 that 11archive accepted against 11ai, but check the pair at 16px before choosing it.
