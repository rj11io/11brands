...
The `11sponsors` site. This file is candidate `emerald600` in the first
11sponsors colour round: twelve light-mode variants, six pink and six green,
none of them chosen yet.

## Palette decision

Mode light. Signal `#059669` on ground `#FAFAFA`: **3.61:1**.

Emerald 600. The passing counterpart of the requested emerald: same direction,
stepped dark enough to clear the floor.
...
| --- | --- | --- |
| ground | `#FAFAFA` | 3.61:1 |
| ink (the numeral beside the square) | `#0A0A0A` | 5.25:1 |
| footer grey | `#676767` | 1.50:1 |

Measured against `11ai`, the brand this half of the round has to clear:
CIEDE2000 **10.1**. Every candidate here is chromatic, so the footer ratio
...
Floor-only at 3.61:1. dE 10.1 from 11ai and dE 14.6 from 11blog's emerald, so
it threads between the family's two existing greens. The candidate to pick if
the appeal of 11blog's green is the hue rather than the brightness.
